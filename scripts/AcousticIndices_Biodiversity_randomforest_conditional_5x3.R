...
control <- trainControl(method = "repeatedcv", number = 5, repeats = 3, verbose = FALSE, savePredictions = TRUE)
...
RandomForestPerformance_cforest <- RandomForestPerformance_cforest %>% mutate(normRMSE = RMSE/(maxResponse-minResponse),
                                                                              ...
                                                                              normRMSE_SE = RMSESD/(maxResponse-minResponse)/sqrt(15),
                                                                              normMAE_SE = MAESD/(maxResponse-minResponse)/sqrt(15),
                                                                              SI_SE = (RMSESD/meanResponse)*100/sqrt(15),
                                                                              Rsquared_SE = RsquaredSD/sqrt(15))
...
ggsave(filename = "outputs/figures/randomforestperformance/Birds_MorningDayAll_cforest_5x3.png",
...
Plot_AllRF <- plot_grid(Plot_AllRF, legend_bottom,
                        ncol = 1, 
                        rel_heights = c(1, 0.1))
ggsave(filename = "outputs/figures/randomforestperformance/AllComparisons_cforest_5x3.png",
       plot = Plot_AllRF,
...
ggsave(filename = "outputs/figures/randomforestobspred/ObservedPredicted_cforest_5x3.png",
...
ggsave(filename = "outputs/figures/randomforestvariableimportance/VariableImportance_cforest_5x3.png",
       plot = Plot_VariableImportance,
       width = 20, height = 20, units = "cm", dpi = 800)

# Save workspace for later loading ----------------------------------------

save.image(file = "outputs/workspaces/RandomForest_cforest_5x3.RData")